...
ebn0ad=10^(.1*ebn0(j));
n0=2*(eb2/ebn0ad);
p2=[50/(66) 50/(66) 50/(66)];
p0=p2;
y=y';
for i=1:3   
k1(i)=((p2(i)+p0(i))*((m+np)*p2(i)))/((p2(i)*p0(i))*(m*y(i)*(eb2/n0)));
k2(i)=(B*(((m+np)*p2(i))^2))/(2*(m^2)*(p2(i)*p0(i))*(y(i)^2)*((eb2/n0)^2));
k3(i)=((2*((m+np)*p2(i)).*I(i))./(m*eb2.*(y(i))));
k4(i)=(((m+np)*p2(i))*I(i)*(n0))./(p2(i).*m.*(y(i).^2)*(eb2/n0));
